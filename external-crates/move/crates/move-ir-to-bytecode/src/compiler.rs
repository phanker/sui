...
use move_binary_format::{
    file_format::{
        ...
        Constant, DataTypeHandleIndex, DataTypeTyParameter, EnumDefinition, EnumDefinitionIndex,
...
pub fn compile_module<'a>(
    ...
    for s in &module.structs {
        ...
        let type_parameters = data_type_type_parameters(&s.value.type_formals);
        context.declare_data_type_handle_index(ident, abilities, type_parameters)?;
    }
...
fn compile_explicit_dependency_declarations(
    ...
    for dependency in dependencies {
        ...
        for data_dep in data_types {
            ...
            let kinds = data_type_type_parameters(&tys);
            context.declare_data_type_handle_index(sname, ability_set, kinds)?;
        }
...
fn data_type_type_parameters(ast_tys: &[ast::DataTypeTypeParameter]) -> Vec<DataTypeTyParameter> {
    ast_tys
        .iter()
...
fn compile_type(
    ...
    Ok(match ty {
        ...
        Type::DataType(ident, tys) => {
            let sh_idx = context.data_type_handle_index(ident.clone())?;
...
fn compile_structs(
    ...
    for s in structs {
        ...
        let sh_idx = context.data_type_handle_index(sident.clone())?;
...
fn compile_enums(
    ...
    for s in enums {
        ...
        let eh_idx = context.data_type_handle_index(sident.clone())?;
...
fn compile_statement(
    ...
    match statement.value {
        ...
        Statement_::UnpackVariant(name, variant_name, tys, bindings, e, unpack_type) => {
            ...
            let eh_idx = context.data_type_handle_index(qname)?;
            let tag = context.variant(eh_idx, variant_name)? as u16;
            if tys.is_empty() {
                let bytecode = match unpack_type {
                    UnpackType::ByValue => Bytecode::UnpackVariant(def_idx, tag),
                    UnpackType::ByImmRef => Bytecode::UnpackVariantImmRef(def_idx, tag),
                    UnpackType::ByMutRef => Bytecode::UnpackVariantMutRef(def_idx, tag),
                };
                push_instr!(statement.loc, bytecode);
            } else {
                let type_parameters_id = context.signature_index(tokens)?;
                let si_idx = context.enum_instantiation_index(def_idx, type_parameters_id)?;
                let bytecode = match unpack_type {
                    UnpackType::ByValue => Bytecode::UnpackVariantGeneric(si_idx, tag),
                    UnpackType::ByImmRef => Bytecode::UnpackVariantGenericImmRef(si_idx, tag),
                    UnpackType::ByMutRef => Bytecode::UnpackVariantGenericMutRef(si_idx, tag),
                };
...
fn compile_jump_table(
    ...
    for (i, (name, lbl)) in lbls.into_iter().enumerate() {
        let tag = context.variant(eh_idx, name.clone())?;
        if i != tag {
...
fn compile_expression(
    ...
    match exp.value {
        ...
        Exp_::Pack(name, ast_tys, fields) => {
            ...
            let sh_idx = context.data_type_handle_index(ident)?;
        ...
        Exp_::Borrow {
            ...
            let sh_idx = context.data_type_handle_index(struct_ident)?;
        ...
        Exp_::PackVariant(name, variant_name, ast_tys, fields) => {
            ...
            let sh_idx = context.data_type_handle_index(ident)?;
            let num_fields = fields.len();
            let tag = context.variant(sh_idx, variant_name)?;
            ...
            if ast_tys.is_empty() {
                push_instr!(exp.loc, Bytecode::PackVariant(def_idx, tag as u16));
            } else {
                let si_idx = context.enum_instantiation_index(def_idx, type_actuals_id)?;
                push_instr!(exp.loc, Bytecode::PackVariantGeneric(si_idx, tag as u16));
            }
...
fn compile_bytecode(
    ...
    let ff_instr = match instr_ {
        ...
        IRBytecode_::MutBorrowField(name, tys, sp!(_, field_)) => {
            ...
            let sh_idx = context.data_type_handle_index(qualified_struct_name)?;
        ...
        IRBytecode_::PackVariant(name, variant_name, tys) => {
            ...
            let eh_idx = context.data_type_handle_index(qualified_enum_name)?;
            let tag = context.variant(eh_idx, variant_name)?;
            if tys.is_empty() {
                Bytecode::PackVariant(def_idx, tag as u16)
            } else {
                let si_idx = context.enum_instantiation_index(def_idx, type_actuals_id)?;
                Bytecode::PackVariantGeneric(si_idx, tag as u16)
            }
        }
...
fn remap_branch_offsets(
    ...
    for jt in jump_tables.iter_mut() {
        for offset in jt.jump_table.iter_mut() {
            *offset = fake_to_actual[offset]
        }
    }
}
