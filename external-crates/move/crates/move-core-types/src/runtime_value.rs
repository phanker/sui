...
impl<'d> serde::de::DeserializeSeed<'d> for &MoveEnumLayout {
    ...
    fn deserialize<D: serde::de::Deserializer<'d>>(
        ...
        let (tag, fields) = deserializer.deserialize_tuple(2, EnumFieldVisitor(&self.0))?;
...
struct MoveVariantFieldLayout<'a>(&'a Vec<MoveTypeLayout>);
...
impl serde::Serialize for MoveVariant {
    ...
    fn serialize<S: serde::Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        ...
        t.serialize_element(&MoveValue::U16(self.tag))?;
