...
impl ModuleCache {
    ...
    fn add_module(
        ...
        for struct_type in self.types.binaries.iter().rev().take(field_types_len) {
            self.calculate_depth_of_data_type(struct_type, &mut depth_cache)?;
        }
    ...
    fn make_type(&self, module: BinaryIndexedView, tok: &SignatureToken) -> PartialVMResult<Type> {
        let res = match tok {
            ...
            SignatureToken::DataType(sh_idx) => {
                let struct_handle = module.data_type_handle_at(*sh_idx);
    ...
    fn calculate_depth_of_data_type(
        &self,
        ...
        let formulas = match &struct_type.data_type_info {
    ...
    fn calculate_depth_of_type(
        ...
        Ok(match ty {
            ...
            Type::DataType(cache_idx) => {
                ...
                let struct_formula =
                    self.calculate_depth_of_data_type(&struct_type, depth_cache)?;
...
impl<'a> Resolver<'a> {
    ...
    pub(crate) fn get_enum_type(&self, idx: EnumDefinitionIndex) -> Type {
        let struct_def = match &self.binary {
            BinaryType::Module { loaded, .. } => loaded.enum_at(idx),
            BinaryType::Script(_) => unreachable!("Scripts cannot have type instructions"),
        };
        Type::DataType(struct_def)
    }
    ...
    pub(crate) fn instantiate_enum_type(
        ...
        let enum_inst = match &self.binary {
            BinaryType::Module { loaded, .. } => loaded.enum_instantiation_at(idx.0),
            BinaryType::Script(_) => unreachable!("Scripts cannot have type instructions"),
        };
    ...
    pub(crate) fn variant_field_count(&self, idx: EnumDefinitionIndex, vtag: VariantTag) -> u16 {
        match &self.binary {
            BinaryType::Module { loaded, .. } => loaded.variant_field_count(idx, vtag),
            BinaryType::Script(_) => unreachable!("Scripts cannot have type instructions"),
        }
    }
    ...
    pub(crate) fn variant_instantiantiation_field_count(
        ...
        match &self.binary {
            BinaryType::Module { loaded, .. } => {
                loaded.variant_instantiantiation_field_count(idx, vtag)
            }
            BinaryType::Script(_) => unreachable!("Scripts cannot have type instructions"),
        }
    }
...
impl LoadedModule {
    fn new(
        ...
        for data_type_handle in module.data_type_handles() {
            let struct_name = module.identifier_at(data_type_handle.name);
            let module_handle = module.module_handle_at(data_type_handle.module);
...
impl LoadedScript {
    fn new(
        ...
        let mut data_type_refs = vec![];
        for data_type_handle in script.data_type_handles() {
            let struct_name = script.identifier_at(data_type_handle.name);
            let module_handle = script.module_handle_at(data_type_handle.module);
            ...
            data_type_refs.push(
                cache
                    .resolve_type_by_name(struct_name, &module_id)
                    .map_err(|e| e.finish(Location::Script))?
                    .0,
            );
        }
        ...
        let jump_tables = script.code.jump_tables.clone();
