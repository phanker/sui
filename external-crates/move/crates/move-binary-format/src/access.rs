...
pub trait ModuleAccess: Sync {
    ...
    fn struct_name(&self, idx: StructDefinitionIndex) -> &IdentStr {
        ...
        let handle = self.data_type_handle_at(struct_def.struct_handle);
        self.identifier_at(handle.name)
    }

    fn enum_name(&self, idx: EnumDefinitionIndex) -> &IdentStr {
        let enum_def = self.enum_def_at(idx);
        let handle = self.data_type_handle_at(enum_def.enum_handle);
        self.identifier_at(handle.name)
    }
    ...
    fn data_type_handle_at(&self, idx: DataTypeHandleIndex) -> &DataTypeHandle {
        let handle = &self.as_module().data_type_handles[idx.into_index()];
    ...
    fn data_type_handles(&self) -> &[DataTypeHandle] {
        &self.as_module().data_type_handles
    }
    ...
    fn find_struct_def_by_name(&self, name: &IdentStr) -> Option<&StructDefinition> {
        self.struct_defs().iter().find(|def| {
            let handle = self.data_type_handle_at(def.struct_handle);
...
pub trait ScriptAccess: Sync {
    ...
    fn data_type_handle_at(&self, idx: DataTypeHandleIndex) -> &DataTypeHandle {
        &self.as_script().data_type_handles[idx.into_index()]
    }
    ...
    fn data_type_handles(&self) -> &[DataTypeHandle] {
        &self.as_script().data_type_handles
    }
